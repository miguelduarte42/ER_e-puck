...
struct HNeuralNetwork **networks;

void HCreateNetworks(int numberOfNetworks)
{
    networks = malloc(sizeof(struct HNeuralNetwork*)*numberOfNetworks);

    int i;
    for(i = 0 ; i < numberOfNetworks ; i++) {
        struct HNeuralNetwork* ann = malloc(sizeof(struct HNeuralNetwork));
        networks[i] = (struct HNeuralNetwork*) ann;
    }
}
...
{    
    ...
    networks[index]->m_pfInputToHiddenWeights  = &pfGenes[0];//malloc((A * B)*sizeof(float));
    networks[index]->m_pfHiddenToHiddenWeights = &networks[index]->m_pfInputToHiddenWeights[A * B];//malloc((B * B)*sizeof(float));
    networks[index]->m_pfHiddenBiases          = &networks[index]->m_pfHiddenToHiddenWeights[B*B];//malloc(B*sizeof(float));

    networks[index]->m_pfHiddenTaus            = &networks[index]->m_pfHiddenBiases[B];//malloc(B*sizeof(float));

    networks[index]->m_pfHiddenToOutputWeights = &networks[index]->m_pfHiddenTaus[B];//malloc((B * C)*sizeof(float));
    networks[index]->m_pfOutputBiases          = &networks[index]->m_pfHiddenToOutputWeights[B * C];//malloc(C*sizeof(float));
